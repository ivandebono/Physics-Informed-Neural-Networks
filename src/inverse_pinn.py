...
class InversePINN(nn.Module):
    def __init__(self, zeta_init: float = 0.2, omega_n_init: float = 3.0) -> None:
        super().__init__()
        ...
        self.zeta = nn.Parameter(torch.tensor([zeta_init], dtype=torch.float64))
        self.omega_n = nn.Parameter(torch.tensor([omega_n_init], dtype=torch.float64))
